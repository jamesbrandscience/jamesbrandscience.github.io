...
<dt>`r fa("folder-open", height = 18)` `screenshots`</dt>
...
You can run a line of code by clickling anywhere on the line and pressing:
...
In R there are also syntatic-style rules for how functions work (please note that natural languages and programming languages are very different, I am just using the terms verbs, nouns, syntax etc for an analogy).
...
PLease, please, please never change the raw file! Going in to Excel/spreadsheet software and changing it in any way will comprimise the integrity of your data.
...
We will not be using the excel version of the data for this worksheet, but will focus on the .csv version. Thi section is to show you how to read in excel files.
...
You should see this (make sure that the **View** has a captical **V**):
...
Pipes are really helpfull and we will be using them throughout this worksheet.
...
Inside the function, we need to specify which varibles we want to remove.
